...
int type;       // type of depot: 2/4/6
int data_type;  // type of instance: SD=0,p/pr=1

int vetex_number;      // num of vetex( = depots num + customer num )
string depot_0;        // instance file name
...
Info readTxt() {
    ...
    if (data_type == 1) {
        ...
        info.vehicle_capacity = stoi(res[2][1]);      // capacity(assume it is same for each vehicle)
        ...
        info.vehicle_capacity = stoi(res[0][1]);      // capacity
        ...
        tot = info.vehicle_capacity * info.vehicle_num;
...
double calculateCost(vector<vector<int>> s) {
    ...
    for (int i = 0; i < s.size(); i++) {
        ...
        for (int j = 0; j < s[i].size() - 1; j++) {
            p = s[i][j];
            q = s[i][j + 1];
            cost = cost + get(info.loc[p], info.loc[q]);
        }
...
int main(int argc, char* argv[]) {
    ...
    // strInputFile = "../dataset/SD_set/k=6/SD11.txt";
    // alg = "GREEDY";
    // strOutputFile = "../output_SD.txt";
    ...
    try {
        ...
        type = info.depots_num;
        vetex_number = info.customer_num + info.depots_num;
        ...
        // printRoute(route);
        double minCost = calculateCost(route);
        ...
        cout << "The run time is:" << time_cost << "(s)" << endl;
        ...
        for (int i = 0; i < route.size(); i++) {
            ...
            for (int j = 0; j < route[i].size(); j++) {
                if (j == 0) {
                    ...
                    cout << setw(5) << route[i][j] << "_" << cus_demand[i][route[i][j]];
